...
function copySections(items: LandingPageSection[]): LandingPageSection[] {
  return items.map((item) => ({ ...item }));
}

function copyTestimonials(items: Testimonial[]): Testimonial[] {
  return items.map((item) => ({ ...item }));
}

function copyEvents(items: AnalyticsEvent[]): AnalyticsEvent[] {
  return items.map((item) => ({ ...item }));
}
...
function findTestimonial(id: string): Testimonial {
  return {
...
export async function listLogos(): Promise<
  ApiResult<LandingPageSection[]>
> {
  return fetchData("/api/fe253b445", () => copySections(mockLogoList));
}

export async function getLogo(
  id: string,
): Promise<ApiResult<LandingPageSection>> {
  return fetchData(`/api/fe253b445/${encodeURIComponent(id)}`, () =>
    findSection(mockLogoList, id),
  );
}
...
export async function listResponsiveSettings(): Promise<
  ApiResult<LandingPageSection[]>
> {
  return fetchData("/api/pc", () => copySections(mockResponsiveSettingList));
}
...
export async function getResponsiveSetting(
  ...
  return fetchData(`/api/pc/${encodeURIComponent(id)}`, () =>
    findSection(mockResponsiveSettingList, id),
  );
}
...
export async function listSeoMetadata(): Promise<
  ApiResult<LandingPageSection[]>
> {
  return fetchData("/api/seo", () => copySections(mockSeoMetadataList));
}
...
export async function getSeoMetadata(
  ...
  return fetchData(`/api/seo/${encodeURIComponent(id)}`, () =>
    findSection(mockSeoMetadataList, id),
  );
}

export async function listTestimonials(): Promise<ApiResult<Testimonial[]>> {
  return fetchData("/api/screen-ff2f57", () =>
    copyTestimonials(mockTestimonialList),
  );
}
...
export async function listCtas(): Promise<
  ApiResult<LandingPageSection[]>
> {
  return fetchData("/api/cta", () => copySections(mockCtaList));
}
...
export async function getCta(
  ...
  return fetchData(`/api/cta/${encodeURIComponent(id)}`, () =>
    findSection(mockCtaList, id),
  );
}

export async function getCtaSummary(): Promise<
  ApiResult<AnalyticsEvent[]>
> {
  return fetchData("/api/cta/summary", () =>
    copyEvents(mockAnalyticsEventList),
  );
}
