...
- **Why**: 사용자는 랜딩 페이지 방문자가 가질 수 있는 일반적인 궁금증을 미리 해소하여 서비스에 대한 이해를 돕고 신뢰를 얻고자 했습니다.
- **Constraint**: AI는 랜딩 페이지의 핵심 서비스 범위(가구 가격 비교 및 기획안 생성)와 일치하지 않는 내용(가구 구매 및 배송 관련 질문)이 추가될 때, 서비스의 정체성이 모호해지거나 잘못된 정보를 제공하는 것을 방지하기 위해 사용자에게 해당 불일치를 명확히 알리고 최종 결정을 요청하도록 시스템이 설계되었습니다.