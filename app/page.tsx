...
export default async function HomePage() {
  ...
  return (
    <ScreenShell route="/">
      ...
      <nav data-cbv-src="app/page.tsx:47" aria-label="랜딩 흐름" className="py-12">
        <Stagger className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {SCREEN_ORDER.map((item, index) => (
            <StaggerItem key={item.route}>
              ...
              >
                <span data-cbv-src="app/page.tsx:55" className="text-sm font-medium text-brand">
                  {String(index + 1).padStart(2, "0")} · {item.label}
                </span>
                <span data-cbv-src="app/page.tsx:58" className="text-base font-semibold break-keep text-foreground">
                  {SCREEN_HEADLINES[item.route]}
                </span>
                <span data-cbv-src="app/page.tsx:61" className="mt-auto flex items-center gap-1 text-sm text-muted-foreground group-hover:text-foreground">
                  보러 가기
                  <ArrowRight
      ...
      <div data-cbv-src="app/page.tsx:74" className="flex flex-col gap-16 py-12">
