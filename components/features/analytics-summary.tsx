...
export function AnalyticsSummary({
  ...
  return (
    <section data-cbv-src="components/features/analytics-summary.tsx:59" data-feat-id="feat_fb0d21267" className="flex flex-col gap-6">
      <FadeIn>
        <h2 data-cbv-src="components/features/analytics-summary.tsx:61" className="text-xl font-semibold tracking-tight text-foreground sm:text-2xl">
          {title}
        </h2>
      </FadeIn>

      {loading ? (
        <div data-cbv-src="components/features/analytics-summary.tsx:67" className="grid grid-cols-1 gap-[var(--density-gap)] sm:grid-cols-2">
          {[0, 1].map((key) => (
            <Card key={key}>
              ...
              <StaggerItem key={tile.key} className="h-full">
                <Card className="h-full">
                  <CardContent className="flex h-full flex-col gap-3 p-6">
                    <p data-cbv-src="components/features/analytics-summary.tsx:91" className="flex items-center gap-2 text-sm text-muted-foreground">
                      <Icon className="size-4 text-brand" aria-hidden="true" />
                      {tile.label}
                    </p>
                    <p data-cbv-src="components/features/analytics-summary.tsx:95" className="text-3xl font-semibold tracking-tight text-brand">
      ...
      {!loading && latest ? (
        <p data-cbv-src="components/features/analytics-summary.tsx:107" className="text-sm text-muted-foreground">
