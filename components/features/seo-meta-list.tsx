...
export function SeoMetaList({
  ...
  return (
    <section data-feat-id="feat_f69e65664" className={cn("flex flex-col gap-6", className)}>
      {loading ? (
            ...
            <StaggerItem key={item.id}>
              <article className="flex flex-col gap-4 rounded-lg border border-border bg-card p-6">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="min-w-0 truncate text-sm text-muted-foreground">{item.path}</span>
                </div>
                <div className="flex flex-col gap-1">
                  <p className="flex items-start gap-2 text-base font-medium break-keep text-foreground">
                    <Type className="mt-1 h-4 w-4 shrink-0 text-brand" aria-hidden="true" />
                    <span className="sr-only">제목: </span>
                    {item.title}
                  </p>
                  <p className="pl-6 text-sm leading-relaxed break-keep text-muted-foreground">
                ...
                <dl className="flex flex-wrap gap-x-6 gap-y-2 text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <dt className="text-muted-foreground">대표 URL</dt>
                    <dd className="font-medium text-foreground">{item.canonicalUrl}</dd>
                  </div>
                  <div className="flex items-center gap-2">
                    <Globe className="h-4 w-4 text-muted-foreground" aria-hidden="true" />
                    <dt className="text-muted-foreground">언어</dt>
                    <dd className="font-medium text-foreground">{item.language === "ko" ? "한국어" : item.language}</dd>
                  </div>
                </dl>
              </article>
            </StaggerItem>
