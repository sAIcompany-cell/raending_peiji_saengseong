...
export function SeoMetaList({ items, loading, error, className }: SeoMetaListProps) {
  return (
    <div data-feat-id="feat_f69e65664" className={cn("flex flex-col gap-6", className)}>
      <div className="flex flex-col gap-1">
        <h2 className="font-heading text-xl font-semibold text-foreground">검색과 공유에서 이렇게 보입니다</h2>
        <p className="text-sm text-muted-foreground">화면마다 고유한 제목과 설명, 대표 주소가 붙어 있습니다.</p>
      </div>
          ...
          {items.map((item) => {
            ...
            return (
              <StaggerItem key={item.id}>
                <article className="flex flex-col gap-2 rounded-lg border border-border bg-card p-6 transition-colors hover:border-brand/40">
                  ...
                  <h3 className="font-heading text-lg font-semibold text-brand">{item.ogTitle ?? item.title}</h3>
                  <p className="text-sm leading-relaxed text-muted-foreground">{item.ogDescription ?? item.description}</p>
