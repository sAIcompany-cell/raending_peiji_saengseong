...
export function SiteLogo({ logo, className }: SiteLogoProps) {
  ...
  return (
    <div data-cbv-src="components/features/site-logo.tsx:17" data-feat-id="feat_fe253b445" className="flex shrink-0 items-center">
      <Link
        href={href}
      ...
      >
        <span data-cbv-src="components/features/site-logo.tsx:27" className="flex size-8 items-center justify-center rounded-md bg-brand text-brand-foreground">
          <Armchair aria-hidden="true" className="size-4" />
        </span>
        <span data-cbv-src="components/features/site-logo.tsx:30" className="whitespace-nowrap">{name}</span>
      </Link>
    </div>
  );
}
