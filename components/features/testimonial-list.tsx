  ...
  return (
    <Card className="h-full">
      <CardContent className="flex h-full flex-col gap-4 p-6">
        ...
        <blockquote className="flex-1 text-base leading-relaxed break-keep text-foreground">
          {item.quote}
        </blockquote>
        {item.result ? (
          <p className="flex items-start gap-2 rounded-md bg-muted p-3 text-sm break-keep text-foreground">
            <CircleCheck className="mt-0.5 h-4 w-4 shrink-0 text-brand" aria-hidden="true" />
            <span>
              <span className="sr-only">결과: </span>
              {item.result}
            </span>
          </p>
        ) : null}
...
export function TestimonialList({
  ...
  return (
    <section data-feat-id="feat_feat-ff2f57" className={cn("flex flex-col gap-8", className)}>
      {title || subtitle ? (
        <div className="flex flex-col gap-3">
          {title ? (
            <h2 className="font-heading text-xl font-semibold tracking-tight break-keep text-foreground sm:text-3xl">
              {title}
            </h2>
          ) : null}
          {subtitle ? (
            <p className="max-w-2xl text-base break-keep text-muted-foreground">{subtitle}</p>
          ) : null}
        </div>
      ) : null}
