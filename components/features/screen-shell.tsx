...
export function ScreenShell({
  ...
  return (
    <div data-cbv-src="components/features/screen-shell.tsx:42" className="flex min-h-dvh flex-col bg-background text-foreground">
      <PageViewTracker pagePath={route} />
      <SiteHeader
        logo={mockLogoList[0]}
        items={SCREEN_ORDER.map((item) => ({ href: item.route, label: item.label }))}
      />
      <main data-cbv-src="components/features/screen-shell.tsx:48" className="flex-1 py-12 sm:py-20">
