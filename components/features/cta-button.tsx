...
export function CtaButton({
  ...
  return (
    <div data-cbv-src="components/features/cta-button.tsx:67" data-feat-id="feat_fd60f2b56" className={cn("flex flex-col items-start gap-3", className)}>
      {badge ? (
      ...
      >
        ...
        <span data-cbv-src="components/features/cta-button.tsx:83">{text}</span>
        {state !== "navigating" ? <ArrowRight className="h-4 w-4" aria-hidden="true" /> : null}
      </Button>

      <p data-cbv-src="components/features/cta-button.tsx:87" aria-live="polite" className="min-h-5 text-sm text-muted-foreground">
        {state === "navigating" ? `'${text}'을(를) 눌렀어요. 다음 화면으로 이동하고 있어요.` : null}
        {inactive && state === "idle" ? "지금은 이 버튼을 사용할 수 없어요." : null}
      </p>

      {state === "error" ? (
        <p data-cbv-src="components/features/cta-button.tsx:93" role="alert" className="flex items-center gap-2 text-sm text-destructive">
