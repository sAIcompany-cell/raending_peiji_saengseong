...
export function ResponsiveSupportList({ items, loading, error, className }: ResponsiveSupportListProps) {
  return (
    <div data-feat-id="feat_fd435a7f2" className={cn("flex flex-col gap-6", className)}>
      <div className="flex flex-col gap-1">
        <h2 className="font-heading text-xl font-semibold text-foreground">어떤 화면에서도 같은 흐름으로 비교하세요</h2>
        <p className="text-sm text-muted-foreground">휴대폰·태블릿·PC 어디서 열어도 내용과 버튼이 잘리지 않습니다.</p>
      </div>
                ...
                <Card className="h-full">
                  <CardContent className="flex h-full flex-col gap-4 p-6">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className={cn("inline-flex items-center gap-1 text-xs font-medium", support.className)}>
