    ...
    <Card className="h-full transition-colors hover:border-brand/40">
      <CardContent className="flex h-full flex-col gap-4 p-6">
        <div className="flex items-center justify-between">
          <span className="inline-flex size-10 items-center justify-center rounded-md bg-accent text-brand">
...
export function SectionNote({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <p className={className ?? "flex items-start gap-2 text-base text-foreground"}>
...
export function SectionPanel({ section, loading, error, step, totalSteps, children }: SectionPanelProps) {
  if (loading) {
    return (
      <div className="flex flex-col gap-10" aria-busy="true" aria-label="불러오는 중">
  ...
  return (
    <section aria-labelledby={`${section.id}-title`} className="flex flex-col gap-10 sm:gap-12">
      <FadeIn className="flex flex-col gap-4">
        ...
        <p className="max-w-2xl text-pretty text-lg text-muted-foreground">{section.subtitle}</p>
      ...
      <FadeIn className="flex flex-col gap-6">
        {progress !== null ? (
          <div className="flex flex-col gap-2" aria-label={`${step}단계 / 총 ${totalSteps}단계`}>
