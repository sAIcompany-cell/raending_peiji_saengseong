...
export function SiteHeader({
  ...
  return (
    <header data-cbv-src="components/features/site-header.tsx:40" className="sticky top-0 z-30 border-b border-border bg-background/90 backdrop-blur">
      <Container size="wide" className="flex flex-wrap items-center gap-3 py-3 sm:gap-4">
        <SiteLogo logo={logo} />

        <nav data-cbv-src="components/features/site-header.tsx:44" aria-label="주요 화면" className="order-3 w-full sm:order-2 sm:w-auto sm:flex-1">
          <ul data-cbv-src="components/features/site-header.tsx:45" className="flex flex-wrap items-center gap-x-4 gap-y-1 sm:gap-x-5">
            {navItems.map((item) => {
              const active = pathname === item.href;
              return (
                <li data-cbv-src="components/features/site-header.tsx:49" key={item.href}>
                  <Link
                    href={item.href}
        ...
        <div data-cbv-src="components/features/site-header.tsx:68" className="order-2 ml-auto sm:order-3 sm:ml-0">
          <Link
            href={ctaHref}
