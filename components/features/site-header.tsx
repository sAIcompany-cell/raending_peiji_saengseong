...
export function SiteHeader({
  ...
  return (
    <header data-cbv-src="components/features/site-header.tsx:38" className="sticky top-0 z-40 w-full border-b border-border bg-background/95 backdrop-blur">
      <Container className="flex h-16 items-center justify-between gap-4">
        <SiteLogo logo={logo} />

        <nav data-cbv-src="components/features/site-header.tsx:42" aria-label="주요 화면" className="hidden items-center gap-1 md:flex">
          {items.map((item) => {
      ...
      {open ? (
        <nav data-cbv-src="components/features/site-header.tsx:77" aria-label="주요 화면(모바일)" className="border-t border-border md:hidden">
