...
export function ScreenNav({ sections, className }: ScreenNavProps) {
  ...
  return (
    <nav data-cbv-src="components/features/screen-nav.tsx:27" aria-label="화면 이동" className={cn("flex flex-wrap items-center gap-1", className)}>
      {ordered.map((section, index) => {
        ...
        return (
          ...
          >
            <span data-cbv-src="components/features/screen-nav.tsx:42"
              aria-hidden="true"
              className={cn(
            ...
            <span data-cbv-src="components/features/screen-nav.tsx:51" className="hidden sm:inline">{shortLabel[section.type]}</span>
          </Link>
        );
      })}
    </nav>
